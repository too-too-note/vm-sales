        ...
        >
          <template v-slot:top>
            ...
            >
              ...
              <v-spacer></v-spacer>
              <v-btn @click="update" color="primary">更新</v-btn>
          ...
          <template v-slot:expanded-item="{ headers, item }">
            <td :colspan="headers.length">
              <v-data-table
                  ...
                  hide-default-footer
              >
                <template v-slot:item.dialogBtn="{item}">
                  <v-btn small @click="openSalesListDialog(item)"><v-icon>mdi-playlist-edit</v-icon></v-btn>
                </template>
                <template v-slot:item.salesQuantityUpdate="{item}">
                  <v-text-field type="number" v-model="updateForm[item.id]" dense hide-details></v-text-field>
                </template>
      ...
      >
        <div style="height:100vh">
          <v-card>
            <v-card-text>
              <sales-input
               :arg-sales-date="dialogForm.salesDate"
               :arg-vending-machine-id="dialogForm.vendingMachineId"
               @callBackUpdate="callBackDialogUpdate"
              ></sales-input>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  name: "SalesUpdate.vue",
  components: {DatePicker, SalesInput},
  data: () => ({
    form: {
  ...
  methods: {
    select() {
      ajax.post("sales-update/select-vm", this.form, (res) => {
        this.out = res.data
        this.updateForm = {}
      })
    },
    update() {
      ...
      for (const id in this.updateForm) {
        updateFormList.push({
          vendingMachineItemSalesId: id,
          salesQuantity: this.updateForm[id]
        })
      }
      ajax.post("sales-update/update-sales", { vmItemSalesList: updateFormList }, (res) => {
    ...
    openSalesListDialog(item) {
      const vm = this.out['vmSalesList'].find(vm => vm['itemList'].find(i => i.id === item.id))
      this.dialogForm.salesDate = item.salesDate
      this.dialogForm.vendingMachineId = vm.vendingMachineId
      this.dialog = true
    },
    callBackDialogUpdate() {
      this.select()
      this.dialog = false
    }
  },
