...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: '売上数量小計', value: 'subtotalQuantity', align: 'end' },
  ...
  methods: {
    select() {
      ajax.post("sales-inquiry-list/select-item", this.form, (res) => {
        this.out = res.data
      })
    },
    allOpen() {
      this.expanded = []
      this.expanded = this.out.itemSalesList
    },
    allClose() {
      this.expanded = []
    },
